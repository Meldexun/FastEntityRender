...
public class OptifineLegacyModelRenderer extends LegacyModelRenderer {

	private boolean isShaders;

	public OptifineLegacyModelRenderer(long initialCapacity) {
	...
	protected void setupAttributePointers(ByteBuffer buffer) {
		super.setupAttributePointers(buffer);
		if (isShaders) {
			GL20.glVertexAttribPointer(Optifine.midTexCoordAttrib, 2, GL11.GL_FLOAT, false, vertexSize, (ByteBuffer) buffer.position(24));
			GL20.glVertexAttribPointer(Optifine.tangentAttrib, 4, GL11.GL_SHORT, false, vertexSize, (ByteBuffer) buffer.position(32));
		}
	}
